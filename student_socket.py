...
class StudentUSocket(StudentUSocketBase):
  ...
  def handle_accepted_fin(self, seg):
    ...
    ## Start of Stage 7.2 ##
    """
    if self.state |EQ| FIN_WAIT_1 and seg.ack |EQ| self.snd.nxt:
      self.set_pending_ack()
      self.start_timer_timewait()
    elif self.state |EQ| FIN_WAIT_1 and seg.FIN |EQ| True:
      self.set_pending_ack()
      self.state = CLOSING
    elif self.state |EQ| FIN_WAIT_2 and seg.FIN |EQ| True:
    ...
    if self.state == FIN_WAIT_1:
      self.set_pending_ack()
      if seg.ack |EQ| self.snd.nxt:
        self.start_timer_timewait()
      else:
        self.state = CLOSING
    elif self.state == FIN_WAIT_2 and seg.ACK: # FIN包的ACK字段也是True
      self.rcv.nxt = self.rcv.nxt |PLUS| 1
      self.set_pending_ack()
      self.start_timer_timewait()
    ## End of Stage 7.2 ##
  ...
  def maybe_send(self):
    ...
    while remaining > 0:
      if not self.tx_data: break
      len_payload = min(remaining, self.mss, len(self.tx_data), self.snd.wnd)
      payload = self.tx_data[:len_payload]
